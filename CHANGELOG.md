...
All three are corrections of errors in the implementation behind the published figures, not alternative conventions. An independent reimplementation should expect exactly these differences, and no other difference from these causes.
...
**Two affect the weights**, and neither reaches a score under the default settings: `weight_source` is `"table1"`, so a run uses the published weights unless it is explicitly pointed at recomputed ones. Both errors have the same shape — an upstream step imposing on the weight calculation a scope it did not choose.

2. **The correlation matrix is built on the active metric set.** The published implementation computed each compartment's matrix over the full 27-metric Walloon superset and then dropped the 7 metrics outside the EU version without recomputing, so metrics the EU version never scores still shaped every weight it kept. The effect tracks how many metrics a compartment loses: honeybees acute moves from 7.5% to 4.6%, while human toxicity barely moves. *(Methodology, section "Weight calculation".)*
...
- **Ranges are resolved rather than discarded.** A cell holding `"10-20"` states an imprecise value, not an unknown one; it resolves to its more hazardous end by default. One such value exists in the 3 May 2024 export.
- **Scores are floored at 0** below the null-hazard threshold, where extrapolation would otherwise produce a negative score. The high-hazard side is left open, so an extreme outlier still scores above 1.50 — which concerns only the four metrics whose score increases with the raw value, the other twelve having their 1.50 threshold at a raw value of 0.
- **GUS is always computed** from the completed soil DT50 and KFOC, never read from a GUS column. No effect on this export, which has none; the rule is unconditional so that a future export cannot silently change the metric's provenance.
- **PPDB placeholders in categorical fields read as missing**, not as a confirmed "no". `"No data"` in a carcinogenicity or neurotoxicity field previously scored as a confirmed absence of effect, which is the opposite of what the field says.
- **Every sheet is read as text**, so that a range, a comparator or a placeholder reaches the parsing rules intact instead of being turned into a missing value by automatic type guessing — and, from there, into a worst-case substitution.
...
- **Weights are renormalised to sum to exactly 1**, the published percentages being rounded.

### Fixed

- Threshold mapping corrected on 7 of the 20 metrics, against table 1 of the article: soil DT50, water DT50, KFOC, GUS, BCF, mammals acute dermal LD50 and mammals acute inhalation LC50. The error was a systematic level-by-level shift, so it produced plausible scores rather than obviously wrong ones — at the regulatory persistence threshold of 180 days, a shifted soil-DT50 mapping scores 0.50 ("moderate") where the published thresholds give 1.00 ("high").

### Known limitations

- The metric list is an **input**, not a computation. Which metrics clear the 60% coverage bar depends on the substance scope and on the data available at the time, and both move; re-deriving it is not implemented.
- Only HPLI-EU is available. The 27-metric Walloon version, and a selectable pair of persistence-coefficient constants, are planned.
- Missing-data imputation from a chemically related substance, or from the mean of a pesticide functional group, is reserved in the traceability schema but not implemented.
- The implementation computes a load score only. Risk scores and the Pesti-Score need use data this code does not read.
- No reference output is included. One is to be published with a later version, computed on a PPDB export under a valid licence.
